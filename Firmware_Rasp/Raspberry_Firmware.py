#!/usr/bin/env python3
"""
Raspberry Pi 5 — mod autonom (implicit): fire dedicate core 2 (senzori + actuatoare + logică),
core 3 (WebSocket TX), core 4 (WebSocket RX). Pornește direct fără meniu.

Meniu de teste (opțional):  python3 hardware_test_menu.py --menu

Dependențe: pip install gpiozero websockets
...
def _sensor_process_loop(shared_state, command_state, stop_event, params: dict) -> None:
    ...
    def servo_set_degrees(deg: int) -> None:
        ...
        def _move_task():
            ...
            # Îi dăm 1.5 secunde să ajungă fizic la poziție
            time.sleep(1.5)
            # Tăiem semnalul PWM exact ca în meniul de test pentru a preveni blocajele
            servo.value = None

        # Rulăm mișcarea într-un fir de execuție separat pentru a nu întârzia citirea DHT22/BMP280
...
class HardwareTestMenu:
    ...
    def default_params() -> dict:
        return {
            ...
            # If true, serial prints: [WS-TX] OUT> telemetrie trimisă la clienți; [WS-RX] IN< comenzi primite.
            "ws_log_traffic": True,
    ...
    def test_buzzer(self) -> None:
        on_sec = float(self.params["actuator_on_sec"])
        # 0.1 inseamna 10% putere (volum redus)
        volum_redus = 0.1

        print(f"\n[BUZZER TEST] ACTIVAT la volum {volum_redus*100}% pentru {on_sec} secunde...")

        self.buzzer.value = volum_redus
        time.sleep(on_sec)
        self.buzzer.value = 0  # Oprit

        self.append_result("buzzer", {"status": "ok", "on_sec": on_sec, "volume": volum_redus})
...
def main() -> int:
    if "--help" in sys.argv or "-h" in sys.argv:
        print(
            "Raspberry Pi 5 hardware (autonom)\n\n"
            "  (implicit)  Core 2 = senzori + logică, Core 3 = WebSocket TX, Core 4 = WebSocket RX\n"
            "  fără meniu; pornește imediat runtime multicore + WebSocket.\n\n"
            "  python3 hardware_test_menu.py        mod autonom (recomandat)\n"
            "  python3 hardware_test_menu.py --menu  meniu interactiv de teste\n"
        )
        return 0
    ...
    try:
        if use_menu:
            ...
            print(
                "========== Mod autonom (fără meniu) ==========\n"
                f"  Core 2 = senzori, LED alb/roșu (connection), throttle, comenzi RX\n"
...
if __name__ == "__main__":
    # FORȚĂM PYTHON SĂ CREEZE PROCESE NOI CURATE, NU CLONATE.
    # Aceasta repară conexiunea PWM pentru servomotor pe Pi 5.
    import multiprocessing as mp
    try:
